        ...
        var interceptor = ['$rootScope', '$q', function($rootScope, $q) {
            ...
            function error(response) {
                ...
                if (response.status === 500) {
                    errorMessage = data.error && data.error.message ? getServerError(data.error.message) : unexpectedError;
                    showError(errorMessage);
                } else if (response.status === 409){
                    errorMessage = data.error && data.error.message ? getServerError(data.error.message) : "Duplicate entry error";
                    showError(errorMessage);
                    ...
                    errorMessage = data.error && data.error.message ? data.error.message : "Could not connect to the server. Please check your connection and try again";
                    showError(errorMessage);
                } else if (response.status === 403) {
                    errorMessage = data.error && data.error.message ? stringAfter(data.error.message, ':') : unexpectedError;
